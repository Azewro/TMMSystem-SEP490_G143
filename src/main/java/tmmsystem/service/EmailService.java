...
public class EmailService {
    ...
    private String generateQuotationEmailHtml(Quotation quotation) {
        ...
        html.append("</head>");
        html.append("<body style='font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 800px; margin: 0 auto; padding: 20px;'>");
        ...
        if (quotation.getRfq() != null && quotation.getRfq().getRfqNumber() != null) {
            html.append("<p style='margin: 5px 0; color: #666;'><strong>Số:</strong> ").append(quotation.getRfq().getRfqNumber()).append("</p>");
        }
        html.append("</div>");
        
        // Title
        html.append("<h2 style='text-align: center; color: #2c3e50; margin: 30px 0; font-size: 20px;'>BẢNG BÁO GIÁ</h2>");
        ...
        if (quotation.getDetails() != null && !quotation.getDetails().isEmpty()) {
            for (QuotationDetail detail : quotation.getDetails()) {
                ...
                html.append("<tr>");
                html.append("<td style='border: 1px solid #ddd; padding: 10px; text-align: center;'>").append(stt++).append("</td>");
                html.append("<td style='border: 1px solid #ddd; padding: 10px;'>");
                html.append("<strong>").append(productName).append("</strong>");
                if (!dimensions.isEmpty()) {
                    html.append("<br><span style='color: #666; font-size: 0.9em;'>Kích thước: ").append(dimensions).append("</span>");
                }
                html.append("</td>");
                html.append("<td style='border: 1px solid #ddd; padding: 10px; text-align: center;'>").append(String.format("%,.0f", quantity.doubleValue())).append("</td>");
                html.append("<td style='border: 1px solid #ddd; padding: 10px; text-align: center;'>").append(String.format("%,.2f", weight.doubleValue())).append("</td>");
                html.append("<td style='border: 1px solid #ddd; padding: 10px; text-align: right;'>").append(String.format("%,.0f", unitPrice.doubleValue())).append(" ₫</td>");
                html.append("<td style='border: 1px solid #ddd; padding: 10px; text-align: right; font-weight: bold;'>").append(String.format("%,.0f", totalPrice.doubleValue())).append(" ₫</td>");
                html.append("</tr>");
            }
        }
        ...
        html.append("<td colspan='3' style='border: 1px solid #ddd; padding: 12px; text-align: right;'>TỔNG CỘNG:</td>");
        html.append("<td style='border: 1px solid #ddd; padding: 12px; text-align: center;'>").append(String.format("%,.2f", totalWeight.doubleValue())).append("</td>");
        html.append("<td style='border: 1px solid #ddd; padding: 12px;'></td>");
        html.append("<td style='border: 1px solid #ddd; padding: 12px; text-align: right; color: #007bff; font-size: 1.1em;'>");
        ...
        if (quotation.getTotalAmount() != null) {
            html.append("<p style='margin: 20px 0;'><em>(Bằng chữ: ").append(numberToWords(quotation.getTotalAmount().longValue())).append(" đồng)</em></p>");
        }
        
        // Notes
        html.append("<div style='margin: 20px 0; padding: 15px; background-color: #fff3cd; border-left: 4px solid #ffc107;'>");
        ...
        // Status
        String statusText = getStatusText(quotation.getStatus());
        String statusColor = getStatusColor(quotation.getStatus());
        html.append("<p style='margin: 10px 0 0 0;'><strong>Trạng thái báo giá:</strong> ");
        html.append("<span style='display: inline-block; padding: 5px 15px; background-color: ").append(statusColor).append("; color: white; border-radius: 3px; font-size: 0.9em;'>").append(statusText).append("</span></p>");
        html.append("</div>");
